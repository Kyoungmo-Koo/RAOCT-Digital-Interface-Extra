...
int test(int argc, char** argv)
{
    ...
    while (1) {
        ...
        if (state == 2 || state == 3) {
            ...
            Sleep(1);
            if (i % 2 == 0) {
                status = SPI_Read(handle, &rx_buffer[0], NUM_OF_BYTES_PER_CHUNK + 4, ptransferCount, options);
            }
            else {
            ...
            if (state == 2 || state == 3) {
                ...
                for (int j = 0; j < MULTIPLIER * NUM_OF_POSITIONS_PER_CHUNK; j += 4) {
