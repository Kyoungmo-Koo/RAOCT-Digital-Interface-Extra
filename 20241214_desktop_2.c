...
int test(int argc, char** argv)
{
    ...
    UCHAR rx_buffer[MULTIPLIER * NUM_OF_POSITIONS_PER_CHUNK + 4];
    ...
    while (1) {
        ...
        if (state == 2 || state == 3) {
            ...
            if (i % 2 == 1) {
                ...
                status2 = SPI_Read(handle, &rx_buffer[0], NUM_OF_BYTES_PER_CHUNK + 4, ptransferCount, options);
                lastBytes = *(uint32_t*)&rx_buffer[MULTIPLIER * NUM_OF_POSITIONS_PER_CHUNK];

                // Print the last 4 bytes in hexadecimal
                // printf("Last 4 bytes received (in hex): 0x%08X\n", lastBytes);
                if (ping == 1) {
                    printf("Ping Result: 0x%08X\n", lastBytes);
                    ping = 0;
                }
            }
