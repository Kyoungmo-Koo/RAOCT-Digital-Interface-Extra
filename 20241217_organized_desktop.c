...
int test(int argc, char** argv)
{
    ...
    const int BYTE_TO_BITS = 4;

    DWORD input_data[CHUNK_NUM * NUM_OF_POSITIONS_PER_CHUNK];
    UCHAR tx_buffer[BYTE_TO_BITS * CHUNK_NUM * NUM_OF_POSITIONS_PER_CHUNK];
    UCHAR tx_message[BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK + BYTE_TO_BITS];
    UCHAR rx_buffer[BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK];
    ...
    while (1) {
        if (_kbhit()) {
            ...
            if (state == 0 && ch == 49) {
                ...
                Sleep(500);
                status = SPI_Write(handle, &tx_buffer[0], BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK * 2, ptransferCount, options);
        ...
        if (state == 2 || state == 3) {
            ...
            Sleep(1);
            status = SPI_Read(handle, &rx_buffer[0], BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK + 4, ptransferCount, options);
            if (ping == 1 && i % 2 == 0) {
                if (print == 0) {
                    print = 1;
                }
                    ...
                    print = 0;
                    ping = 0;
                    printf("Ping ID result: %d, %d, %d, %d \n", rx_buffer[BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK], rx_buffer[BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK + 1], rx_buffer[BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK + 2], rx_buffer[BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK + 3]);
                }
            }
            Sleep(1);
            ...
            if (i % 2 == 1) {
                status = SPI_Write(handle, &tx_buffer[CHUNK_NUM_CURRENT * BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK], BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK, ptransferCount, options);
            }
            else {
                if (state == 2 && idle_sent == 0) {
                    memcpy(tx_message, &tx_buffer[CHUNK_NUM_CURRENT * BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK], BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK);
                    UCHAR extra_bytes[4] = { 0x02, 0x00, 0x00, 0x00 };
                    if (ping == 1) {
                        extra_bytes[0] = 0x04;
                    }
                    memcpy(&tx_message[BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK], extra_bytes, 4);
                    status = SPI_Write(handle, tx_message, BYTE_TO_BITS * NUM_OF_POSITIONS_PER_CHUNK + 4, ptransferCount, options);
            ...
            if (state == 2 || state == 3) {
                ...
                for (int j = 0; j < sizeof(rx_buffer) - BYTE_TO_BITS; j += BYTE_TO_BITS) {
